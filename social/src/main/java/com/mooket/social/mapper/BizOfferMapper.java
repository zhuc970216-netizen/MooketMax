...
public interface BizOfferMapper extends BaseMapper<BizOffer> {
    ...
    @Select({"<script>SELECT o.product_id AS productId, MAX(p.product_name) AS productName, COUNT(*) as totalOfferCount, COUNT(DISTINCT o.merchant_id) as merchantCount, COUNT(DISTINCT o.country || '_' || o.factory_no) as factoryCount, MIN(o.price) as priceMin, MAX(o.price) as priceMax FROM biz_offer o LEFT JOIN dict_product p ON o.product_id = p.product_id WHERE o.status = 'ACTIVE' AND o.data_date >= CURRENT_DATE - INTERVAL '1 day' AND o.offer_type = '报盘' AND o.product_id IS NOT NULL AND (o.price IS NULL OR (o.price > 0 AND o.price &lt;= 200)) <if test='category != null'> AND o.category = #{category} </if>GROUP BY o.product_id</script>"})
    ...
    @Select({"<script>SELECT o.product_id, o.product_name, COALESCE(MIN(CASE WHEN o.price &gt; 0 THEN o.price END), 0) as price_min, COALESCE(MAX(CASE WHEN o.price &gt; 0 THEN o.price END), 0) as price_max, COUNT(DISTINCT o.merchant_id) as merchant_count, COUNT(*) as offer_count, STRING_AGG(DISTINCT CASE WHEN m.merchant_short_name IS NOT NULL AND m.merchant_short_name != '' THEN CONCAT(m.merchant_short_name, '|', m.merchant_name) ELSE COALESCE(m.merchant_name, '') END, ',') as merchant_names FROM biz_offer o LEFT JOIN dict_merchant m ON o.merchant_id = m.merchant_id WHERE o.status = 'ACTIVE' AND o.data_date &gt;= CURRENT_DATE - INTERVAL '1 day' AND o.country = #{country} AND o.factory_no = #{factoryNo} <if test='category != null'> AND o.category = #{category} </if><if test='offerType != null'> AND o.offer_type = #{offerType} </if>AND (o.price IS NULL OR (o.price &gt; 0 AND o.price &lt;= 200)) GROUP BY o.product_id, o.product_name <choose><when test='sortBy == \"price_asc\"'>ORDER BY CASE WHEN MIN(CASE WHEN o.price &gt; 0 THEN o.price END) IS NULL THEN 1 ELSE 0 END ASC, MIN(CASE WHEN o.price &gt; 0 THEN o.price END) ASC, COUNT(*) DESC</when><when test='sortBy == \"price_desc\"'>ORDER BY CASE WHEN MAX(CASE WHEN o.price &gt; 0 THEN o.price END) IS NULL THEN 1 ELSE 0 END ASC, MAX(CASE WHEN o.price &gt; 0 THEN o.price END) DESC, COUNT(*) DESC</when><otherwise>ORDER BY offer_count DESC, MIN(CASE WHEN o.price &gt; 0 THEN o.price END) ASC</otherwise></choose> LIMIT #{limit} OFFSET #{offset}</script>"})
    ...
    @Select({"<script>SELECT COUNT(DISTINCT product_id) FROM biz_offer WHERE status = 'ACTIVE' AND data_date >= CURRENT_DATE - INTERVAL '1 day' AND country = #{country} AND factory_no = #{factoryNo} <if test='category != null'> AND category = #{category} </if><if test='offerType != null'> AND offer_type = #{offerType} </if>AND (price IS NULL OR (price > 0 AND price &lt;= 200)) </script>"})
    ...
    @Select({"<script>SELECT COUNT(DISTINCT COALESCE(merchant_id, -1)) FROM biz_offer WHERE status = 'ACTIVE' AND data_date &gt;= CURRENT_DATE - INTERVAL '1 day' <choose><when test='country != null and country.length() > 0'> AND country = #{country} </when><otherwise> AND (country IS NULL OR country = '') </otherwise></choose><choose><when test='factoryNo != null and factoryNo.length() > 0'> AND REPLACE(factory_no, ' ', '') = REPLACE(#{factoryNo}, ' ', '') </when><otherwise> AND (factory_no IS NULL OR REPLACE(factory_no, ' ', '') = '') </otherwise></choose> <if test='productName != null and productName.length() > 0'> AND product_name = #{productName} </if> <if test='category != null'> AND category = #{category} </if><if test='offerType != null'> AND offer_type = #{offerType} </if></script>"})
    ...
    @Select({"<script>SELECT COUNT(*) FROM biz_offer WHERE status = 'ACTIVE' AND data_date >= CURRENT_DATE - INTERVAL '1 day' <choose><when test='country != null and country.length() > 0'> AND country = #{country} </when><otherwise> AND (country IS NULL OR country = '') </otherwise></choose><choose><when test='factoryNo != null and factoryNo.length() > 0'> AND REPLACE(factory_no, ' ', '') = REPLACE(#{factoryNo}, ' ', '') </when><otherwise> AND (factory_no IS NULL OR REPLACE(factory_no, ' ', '') = '') </otherwise></choose> <if test='productName != null and productName.length() > 0'> AND product_name = #{productName} </if> <if test='category != null'> AND category = #{category} </if><if test='offerType != null'> AND offer_type = #{offerType} </if></script>"})
    ...
    @Select({"<script>SELECT COUNT(DISTINCT product_id) FROM biz_offer WHERE status = 'ACTIVE' AND data_date &gt;= CURRENT_DATE - INTERVAL '1 day' <if test='brandIds != null and brandIds.size() > 0'> AND brand_id IN <foreach collection='brandIds' item='id' open='(' separator=',' close=')'>#{id}</foreach> </if> <if test='category != null'> AND category = #{category} </if> <if test='offerType != null'> AND offer_type = #{offerType} </if></script>"})
    ...
    @Select({"<script>SELECT COUNT(DISTINCT product_id) FROM biz_offer WHERE status = 'ACTIVE' AND data_date &gt;= CURRENT_DATE - INTERVAL '1 day' AND product_name = #{productName} <if test='category != null'> AND category = #{category} </if> <if test='offerType != null'> AND offer_type = #{offerType} </if></script>"})
